...
public class Player4 extends Player {
	...
	protected void start() {
		...
		LOOP: while (!isClear() && !isGameOver()) {
			playSafely(flag);
			System.out.println("仮定1スタート");
			boolean notopen[][] = notOpen(flag);
			for (int y = 0; y < getHeight(); y++) {
				for (int x = 0; x < getWidth(); x++) {
					proflag[x][y] = flag[x][y];
				}
			}
			for (int y = 0; y < getHeight(); y++) {
				for (int x = 0; x < getWidth(); x++) {
					falsefield[x][y] = getCell(x, y);
				}
			}
			for (int x = 0; x < getWidth(); x++) {
				for (int y = 0; y < getHeight(); y++) {
					if (notopen[x][y] && !flag[x][y]) {
						...
						if (opennum == 0) {
							System.out.println(opennum + " " + flagnum + "makeflag or safeopen失敗" + x + " " + y);
						} else {
							for (int s = 0; s < getWidth(); s++) {
								for (int t = 0; t < getHeight(); t++) {
									if (mujuncheck(s, t, proflag)) {
				...
				for (int y = 0; y < getHeight(); y++) {
					if (notopen[x][y] && !flag[x][y]) {
						...
						for (int a = 0; a < 10; a++){
							...
							if (opennum ==0) {
								System.out.println(x + "  " + y + "  開き仮定失敗");
								falsefield[x][y] = getCell(x, y);
							} else {
								for (int s = 0; s < getWidth(); s++) {
									for (int t = 0; t < getHeight(); t++) {
										if (mujuncheck(s, t, proflag)) {
											System.out.println("仮定2成功　　open x" + x + "  y" + y);
											flag[x][y] = true;
											continue LOOP;
										}
									}
	...
	protected void makeFlag3(int x, int y, boolean[][] flag) { // (x,y)周りの、爆弾があるところをtrueとする
		int[] a = check(x, y);
		...
		if (flagNum(x, y, flag) == getCell(x, y)) {
			for (int i = 0; i < 8; i++) {
				if (b[2 * i] < 0 || b[2 * i + 1] < 0)
					...
					if (!flag[b[2 * i]][b[2 * i + 1]] && falsefield[b[2 * i]][b[2 * i + 1]] != 99) {
						// System.out.println(b[2 * i]+" "+b[2 * i + 1]);
						falsefield[b[2 * i]][b[2 * i + 1]] = 99;
						opennum += 1;
					}
				}
			}
	...
	protected boolean mujuncheck(int x, int y, boolean flag[][]) {
		if (getCell(x, y) > 0) {
			if (flagNum(x, y, flag) > getCell(x, y)) {
				return true;
			} else
